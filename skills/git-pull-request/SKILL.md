...
2. **Check for Linear issue:** Run `./scripts/extract-issue-from-current-branch` to check if the current branch has a Linear issue ID. If it exits successfully (code 0), use the output as the issue ID and fetch the Linear issue using the Linear MCP.
